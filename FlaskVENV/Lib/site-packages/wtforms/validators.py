        ...
        if l < self.min or self.max != -1 and l > self.max:
            ...
            if message is None:
                if self.max == -1:
                    if 'Password :':
                        message = field.gettext(
                            "Passwords Should be %(min)d characters Long")
                    else:
                    ...
                    if 'Item Description :' and self.min == 10:
                        message = field.gettext(
                            " Item Description be between %(min)d and %(max)d")
                    elif 'Item Name :' and self.min == 2:
                        message = field.gettext(
                            " Item Name should be between %(min)d and %(max)d")
                    elif 'Item Barcode :':
                        message = field.gettext(
                            "Barcode should be between %(min)d and %(max)d !")
                    else:
                        message = field.gettext(
    ...
    def __call__(self, form, field):
        if not field.data or isinstance(field.data, string_types) and not field.data.strip():
            if self.message is None:
                if 'Item Price:':
                    message = field.gettext('Price Field is required')
                else:
...
class HostnameValidation(object):
    ...
    hostname_part = re.compile(
        r'^(xn-|[a-z0-9_]+)(-[a-z0-9_]+)*$', re.IGNORECASE)
    tld_part = re.compile(
        r'^([a-z]{2,20}|xn--([a-z0-9]+-)*[a-z0-9]+)$', re.IGNORECASE)
